...
class UserController extends Controller
{
    ...
    public function store(UserRequest $request) {
        $user = User::create([
            ...
            'first_name' => ($request->input('first_name')) ? $request->input('first_name') : null,
            'last_name' => ($request->input('last_name')) ? $request->input('last_name') : null,
            'cell_phone' => ($request->input('cell_phone')) ? $request->input('cell_phone') : null,
            'fax' => ($request->input('fax')) ? $request->input('fax') : null,
            'address' => ($request->input('address')) ? $request->input('address') : null,
            'postcode' => ($request->input('postcode')) ? $request->input('postcode') : null,
            'province' => ($request->input('province')) ? $request->input('province') : null,
            'city' => ($request->input('city')) ? $request->input('city') : null,
            'nation' => ($request->input('nation')) ? $request->input('nation') : null,
            'ibernate' => ($request->input('ibernate')) ? $request->input('ibernate') : null,
            'notify' => ($request->input('notify')) ? $request->input('notify') : null,
            'subscribed' => true
        ]);
